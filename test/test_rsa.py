...
class TestRSA(unittest.TestCase):
  ...
  def test_rsa(s):
    ...
    rsa = RSA(7, 17*19)
    s.assertEqual(rsa.to_pem(), b'-----BEGIN PUBLIC KEY-----\nMBswDQYJKoZIhvcNAQEBBQADCgAwBwICAUMCAQc=\n-----END PUBLIC KEY-----')
    rsa2 = RSA.import_pem(b'-----BEGIN PUBLIC KEY-----\nMBswDQYJKoZIhvcNAQEBBQADCgAwBwICAUMCAQc=\n-----END PUBLIC KEY-----')
    ...
    m = int("030379ada3f4f7cff7c114fa625abe9a4bbb457eda3f3f58838f116a01300f7d44886c8639e41c52024e12c31bb6d96f", 16)
  ...
  def test_wiener(s):
    ...
    rsa = RSA(e, n)
    rsa2 = rsautil.wiener(rsa)
    s.assertEqual(rsa2.d, 21780352155588618020563641971337344243907391969899764877790673891831527301137)
  ...
  def test_modulus_fault_crt(s):
    ...
    if check("gp") or check("fplll") or check("gap"):
      ...
      e = 65537
      rsa = RSA(e, n)
      fault_sigs = [1058535326842046404366164623977343348220096515298415971420,
