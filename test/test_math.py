...
class TestContfrac(unittest.TestCase):
    ...
    def test_contfrac(s):
        s.assertEqual(
            contfrac.rational_to_contfrac(10007, 65537),
            [0, 6, 1, 1, 4, 1, 1, 2, 3, 1, 1, 1, 1, 2, 1, 2],
        )
        s.assertEqual(
            contfrac.contfrac_to_rational(
                [0, 6, 1, 1, 4, 1, 1, 2, 3, 1, 1, 1, 1, 2, 1, 2]
            ),
            (10007, 65537),
        )
        s.assertEqual(
            list(
                contfrac.convergents_from_contfrac(
                    [0, 6, 1, 1, 4, 1, 1, 2, 3, 1, 1, 1, 1, 2, 1, 2]
                )
            ),
            [
...
class TestLattice(unittest.TestCase):
    ...
    def test_lattice(s):
        ...
        if check("gp"):
            s.assertEqual(
                lattice.LLL(X_parigp, impl="parigp"), [[0, 1, 0], [1, 0, 1], [-1, 0, 2]]
            )
        if check("fplll"):
            s.assertEqual(
                lattice.LLL(X, impl="fplll"), [[1, -1, 0], [-1, 0, 1], [1, 1, 1]]
            )
        if check("gap"):
            s.assertEqual(
                lattice.LLL(X, impl="gap"), [[1, -1, 0], [0, -1, 1], [1, 1, 1]]
            )
